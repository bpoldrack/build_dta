	...
	for root, dirs, files in os.walk( inputPath ):
		...
		for dir in dirs:
			...
			for file in sorted( os.listdir( dir ) ):
				...
				if magnitude1_time != no_value and magnitude2_time != no_value:
					if ( magnitude1_time > magnitude2_time ):
						print ( magnitude1_time + " was actually later then " + magnitude2_time )
						os.rename(magnitude1_json, dir + temp)
						os.rename(magnitude2_json, magnitude1_json)
						os.rename(dir + temp, magnitude2_json)

                                                os.rename(magnitude1_json.replace(".json", ".nii.gz"), dir + temp)
                                                os.rename(magnitude2_json.replace(".json", ".nii.gz"), magnitude1_json.replace(".json", ".nii.gz"))
                                                os.rename(dir + temp, magnitude2_json.replace(".json", ".nii.gz"))

				if phase1_time != no_value and phase2_time != no_value:
					if (     phase1_time >     phase2_time ):
						print (     phase1_time + " was actually later then " +     phase2_time )
						os.rename(phase1_json, dir + temp)
						os.rename(phase2_json, phase1_json)
						os.rename(dir + temp, phase2_json)

						os.rename(phase1_json.replace(".json", ".nii.gz"), dir + temp)
